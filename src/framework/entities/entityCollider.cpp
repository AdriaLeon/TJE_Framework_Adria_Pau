...
void EntityCollider::testCollision(Matrix44 model, Vector3 center, std::vector<sCollisionData>& WallCollisions, std::vector<sCollisionData>& GroundCollisions) {
	...
	// Floor collisions
	if (mesh->testRayCollision(model, center, Vector3(0.0, -1.0, 0.0) , colPoint, colNormal, player_height / 2 + 0.1f)) {//}, true)) {
	...
	for (int i = 0; i < 2; ++i) {
		//for (const auto& dir : directions) {
			...
			if (mesh->testSphereCollision(model, center/*sphereCenter*/, sphereRadius, colPoint, colNormal)) {
		...
		//}
		center = center + Vector3(0.0f, player_height / 2, 0.0f);
	}
}
...
{
	Vector3 center = target_position + Vector3(0.0f, player_height / 2, 0.0f);
